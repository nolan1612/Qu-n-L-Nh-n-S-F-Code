...
void viewMemberHistory(Event events[], int count) {
    char mssvInput[50];
    printf("\n--- LICH SU THAM GIA CUA THANH VIEN ---\n");
    printf("Nhap MSSV can tra cuu: ");
    scanf(" %[^\n]", mssvInput);
    toUpperCaseStr(mssvInput);
    printf("\nLich su tham gia cua nhan su: %s\n", mssvInput);
    printf("%-10s | %-25s | %-10s | %-15s\n", "Ma ID", "Ten Su Kien", "Vai Tro", "Trang Thai");
    printf("-----------------------------------------------------------------------\n");
    int found = 0;
    for (int i = 0; i < count; i++) {
        for (int j = 0; j < events[i].staffCount; j++) {
            if (strcmp(events[i].staffList[j].studentId, mssvInput) == 0) {
                found = 1;

                char roleStr[15];
                if (events[i].staffList[j].role == 0) strcpy(roleStr, "Leader");
                else if (events[i].staffList[j].role == 1) strcpy(roleStr, "Member");
                else strcpy(roleStr, "Support");

                char statusStr[20];
                if (events[i].status == 0) strcpy(statusStr, "Chua bat dau");
                else if (events[i].status == 1) strcpy(statusStr, "Dang dien ra");
                else strcpy(statusStr, "Ket thuc");
    ...
    if (!found) {
        printf(">> Thong bao: Nhan su %s chua tham gia su kien nao trong ky.\n", mssvInput);
    }
    printf("-----------------------------------------------------------------------\n");
}